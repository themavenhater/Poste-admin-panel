...
function ArticlesCount() {
  ...
  const {articles} = useSelector(({statArticles}) => statArticles.articlesReducer);

  const totalViews = articles.reduce((acc, cur) => [...acc, ...cur.views], [])
      ...
      <div className="widget flex w-full sm:w-1/3 md:w-1/3 p-12">
        <Paper className="w-full rounded-8 shadow-none border-1">
          <div className="text-center pt-12 pb-28">
            <Typography
              className="text-60 leading-none text-blue">{articles.length}</Typography>
            <Typography className="text-16" color="textSecondary">N° articles</Typography>
          </div>
        </Paper>
      </div>
      <div className="widget flex w-full sm:w-1/3 md:w-1/3 p-12">
...
export default withReducer('statArticles', reducer)(ArticlesCount)
